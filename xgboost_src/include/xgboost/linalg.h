    ...
    namespace linalg
    {
        namespace detail
        {
            ...
            struct ArrayInterfaceHandler
            {
                ...
                {
                    return (std::is_floating_point<T>::value ? 'f' : (std::is_integral<T>::value ? (std::is_signed<T>::value ? 'i' : 'u') : '\0'));
                }
            };
...
#pragma unroll n
            ...
            {
                size_t index[D]{0};
                static_assert(std::is_signed<decltype(D)>::value, "Don't change the type without changing the for loop.");
        ...
        private:
            ...
            LINALG_HD size_t MakeSliceDim(size_t new_shape[D], size_t new_stride[D], detail::RangeTag<I> &&range) const
            {
            ...
            LINALG_HD size_t MakeSliceDim(size_t new_shape[D], size_t new_stride[D], detail::RangeTag<I> &&range, S &&...slices) const
            {
                static_assert(new_dim < D, "");
                static_assert(old_dim < kDim, "");
                new_stride[new_dim] = stride_[old_dim];
                ...
                auto offset = stride_[old_dim] * range.beg;
                return MakeSliceDim<old_dim + 1, new_dim + 1, D>(new_shape, new_stride, std::forward<S>(slices)...) + offset;
            }
            ...
            LINALG_HD size_t MakeSliceDim(size_t new_shape[D], size_t new_stride[D], detail::AllTag, S &&...slices) const
            {
                static_assert(new_dim < D, "");
                static_assert(old_dim < kDim, "");
                new_stride[new_dim] = stride_[old_dim];
                new_shape[new_dim] = shape_[old_dim];
                return MakeSliceDim<old_dim + 1, new_dim + 1, D>(new_shape, new_stride, std::forward<S>(slices)...);
            }

            template<size_t old_dim, size_t new_dim, int32_t D, typename Index>
            LINALG_HD size_t MakeSliceDim(DMLC_ATTRIBUTE_UNUSED size_t new_shape[D], DMLC_ATTRIBUTE_UNUSED size_t new_stride[D], Index i) const
            {
                static_assert(old_dim < kDim, "");
            ...
            LINALG_HD std::enable_if_t<std::is_integral<Index>::value, size_t> MakeSliceDim(size_t new_shape[D], size_t new_stride[D], Index i, S &&...slices) const
            {
                static_assert(old_dim < kDim, "");
                auto offset = stride_[old_dim] * i;
                auto res = MakeSliceDim<old_dim + 1, new_dim, D>(new_shape, new_stride, std::forward<S>(slices)...);
                return res + offset;
            }
        ...
        public:
             ...
             */
            template<typename I, int32_t D>
            LINALG_HD TensorView(common::Span<T> data, I const (&shape)[D], int32_t device) : data_{data}, ptr_{data_.data()}, device_{device}
            {
             ...
             */
            template<typename I, int32_t D>
            LINALG_HD TensorView(common::Span<T> data, I const (&shape)[D], I const (&stride)[D], int32_t device) : data_{data}, ptr_{data_.data()}, device_{device}
            {
            ...
            LINALG_HD TensorView(TensorView<U, kDim> const &that) : data_{that.Values()}, ptr_{data_.data()}, size_{that.Size()}, device_{that.DeviceIdx()}
            {
            ...
            LINALG_HD auto Slice(S &&...slices) const
            {
                ...
                // fields.
                TensorView<T, kNewDim> ret{data_.subspan(data_.empty() ? 0 : offset), new_shape, new_stride, device_};
                return ret;
            }
            ...
            {
                return common::Span<size_t const, kDim>{stride_};
            }
        ...
        {
            return MakeVec(data->DeviceIdx() == -1 ? data->HostPointer() : data->DevicePointer(), data->Size(), data->DeviceIdx());
        }

        template<typename T>
        auto MakeVec(HostDeviceVector<T> const *data)
        {
            return MakeVec(data->DeviceIdx() == -1 ? data->ConstHostPointer() : data->ConstDevicePointer(), data->Size(), data->DeviceIdx());
        }
        ...
        public:
             ...
             */
            template<typename I, int32_t D>
            explicit Tensor(I const (&shape)[D], int32_t device) : Tensor{common::Span<I const, D>{shape}, device}
            {}
            ...
            void ModifyInplace(Fn &&fn)
            {
                ...
                CHECK_EQ(this->Data()->Size(), detail::CalcSize(this->shape_)) << "Inconsistent size after modification.";
            }
