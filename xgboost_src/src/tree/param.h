...
namespace xgboost
{
    namespace tree
    {
        ...
        struct TrainParam : public XGBoostParameter<TrainParam>
        {
            ...
            bst_node_t MaxNodes() const
            {
                ...
                else
                {
                    ...
                    CHECK_LE(this->max_depth, 30) << "max_depth can not be greater than 30 as that might generate 2^31 - 1 nodes.";
        ...
        public:
            XGBOOST_DEVICE GradType GetGrad() const
            {
                return sum_grad;
            }

            XGBOOST_DEVICE GradType GetHess() const
            {
                return sum_hess;
            }
            ...
            XGBOOST_DEVICE explicit GradStats(const GpairT &sum) : sum_grad(sum.GetGrad()), sum_hess(sum.GetHess())
            {}
        ...
        struct SplitEntryContainer
        {
            ...
            {
                if (std::isinf(new_loss_chg))
                {  // in some cases new_loss_chg can be NaN or Inf,
             ...
             * \return whether the proposed split is better and can replace current split
             */
            bool Update(bst_float new_loss_chg, unsigned split_index, bst_float new_split_value, bool default_left, bool is_cat, const GradientT &left_sum, const GradientT &right_sum)
            {
            ...
            inline static void Reduce(SplitEntryContainer &dst, const SplitEntryContainer &src)
            { // NOLINT(*)
                dst.Update(src);
            }
        };
