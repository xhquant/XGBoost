...
int DataIterator_Next(DataIterHandle handle)
{
    ...
    char array[] = "{\"data\": [%lu, false], \"shape\":[%lu, 1], \"typestr\": \"<f4\", \"version\": 3}";
     ...
     * iteration or reset) */
    safe_xgboost(XGDMatrixSetDenseInfo(self->_proxy, "label", self->labels[self->cur_it], self->lengths[self->cur_it], 1));
    self->cur_it++;
    return 1;  /* Continue. */
}
...
int main()
{
    ...
    DMatrix Xy;
    safe_xgboost(XGDMatrixCreateFromCallback(&iter, iter._proxy, DataIterator_Reset, DataIterator_Next, config, &Xy));
