...
{
    char const template[] = "{\"data\": [%lu, true], \"shape\": [%lu, %lu], \"typestr\": \"<f4\", \"version\": 3}";
...
size_t Matrix_NSamples(Matrix self)
{
    return self->shape[0];
}

size_t Matrix_NFeatures(Matrix self)
{
    return self->shape[1];
}
...
int main()
{
    ...
    safe_xgboost(XGBoosterLoadModel(booster, "model.json"));
    {
        ...
        char const config[] = "{\"training\": false, \"type\": 0, \"iteration_begin\": 0, \"iteration_end\": 0, \"strict_shape\": true}";
        ...
        safe_xgboost(XGBoosterPredictFromDMatrix(booster, Xy, config, &out_shape, &out_dim, &out_results));
...

    {
        ...
        char const config[] = "{\"type\": 0, \"iteration_begin\": 0, \"iteration_end\": 0, \"strict_shape\": true, \"cache_id\": 0, \"missing\": NaN}";
        /* Shape of output prediction */
        uint64_t const *out_shape;
        /* Dimension of output prediction */
        uint64_t out_dim;
        ...
        char const *X_interface = Matrix_ArrayInterface(X);
        safe_xgboost(XGBoosterPredictFromDense(booster, X_interface, config, NULL, &out_shape, &out_dim, &out_results));

        if (out_dim != 2 || out_shape[0] != N_SAMPLES || out_shape[1] != 1)
        {
            fprintf(stderr, "Regression model should output prediction as vector, %lu, %lu", out_dim, out_shape[0]);
            exit(-1);
        }
